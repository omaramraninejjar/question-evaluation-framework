"""
Concept Coverage dimension (Pedagogical aspect).

Definition:
    Breadth and depth of domain concepts addressed by the item
    relative to the target learning objectives or subject syllabus.

Metrics (all scored against context.course_content):
    Always loaded:
...
class ConceptCoverageDimension(BaseDimension):
    name = DimensionName.CONCEPT_COVERAGE
    description = (
        "Breadth and depth of domain concepts addressed by the item "
        "relative to the target learning objectives or subject syllabus."
    )
